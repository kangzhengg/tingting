...
import { COLORS, MARKERS, CHARACTER_TYPES, CHARACTER_STYLES, CHARACTER_AGES } from '../constants';
...
interface CustomizeProps {
  ...
  updateCharacterAge: (age: AppState['characterAge']) => void;
  t: Translations;
}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
        ...
        <section className="bg-white p-8 rounded-[2.5rem] shadow-xl border border-gray-50 flex flex-col items-center justify-center">
          ...
          <div className="w-full space-y-6">
            {/* Age Toggle */}
            <div>
              <label className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3 block">{t.ageHeader}</label>
              <div className="grid grid-cols-3 gap-3">
                 {CHARACTER_AGES.map(age => (
                   <button
                     key={age.id}
                     onClick={() => updateCharacterAge(age.id as any)}
                     className={`py-3 rounded-2xl font-bold flex flex-col items-center justify-center gap-1 border-2 transition-all ${
                       appState.characterAge === age.id 
                       ? 'bg-gray-900 text-white border-gray-900 shadow-lg scale-105' 
                       : 'bg-gray-50 text-gray-500 border-transparent hover:bg-gray-100'
                     }`}
                   >
                     <span className="text-xl">{age.icon}</span> 
                     <span className="text-[10px]">{t[age.labelKey as keyof Translations]}</span>
                   </button>
                 ))}
              </div>
            </div>
